...
$(function(){
	$("#oldpassword").change(function(){
		...
		$.post("./user.php?control=json&action=checkpwd",{id:$('#id').val(),password:$('#oldpassword').val()},function(data){
			//alert(data) ;
			if(data==0){
				alert("旧密码错误，请重新输入。") ;
				$("#oldpassword").val("") ;
	...
	$("#pwdsubmit").click(function(){
		if(confirm('你确定要提交数据吗？')){
			...
			$.post("./user.php?control=json&action=pwdSubmit",{id:$('#id').val(),password:$('#password').val()},function(data){
				//alert(data) ;
				if(data==1){
					alert("密码修改成功。") ;
				} else {
