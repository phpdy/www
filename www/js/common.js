function btn() {//报名表内容校验
  if(!document.myForm.instruction.checked){
	alert("请接受服务条款");
	return false;
  }
  ...
  if(myForm.realname.value==""){
	...
	if(!chineseReg.test(document.myForm.realname.value)) {
			alert("请填写正确的中文姓名");
  ...
  if(myForm.idcard.value.length!="18"||isNaN(myForm.idcard.value)){
    alert("请输入正确的身份证号码");
    myForm.idcard.focus();
    return false;
  }
  ...
  if(myForm.mobile.value==""){
	...
	if(!phoneNumReg.test(document.myForm.mobile.value)) {
		  alert("请填写正确手机号码!");
  ...
  if(myForm.idcard.value.length!="18"||isNaN(myForm.idcard.value)){
	...
	var TextVal = document.myForm.email.value;           
	var Regex = /^(?:\w+\.?)*\w+@(?:\w+\.)*\w+$/;　
...
　}
  ...
  if(confirm('你确定要提交数据吗？')){
    document.myForm.action="apply.php";
...
function del(id) {
	var applyid = id
	if(confirm("是否确认要删除？")) {
		window.location.href="admin.php?state=0&id="+applyid;
		return true;
	}
	return false;
}
...
function OpYear(){
  for( var i=2013;i>=1940;i--){
    ...
    document.getElementById("st_01").appendChild(List);
    if (i==2013){
  ...
  for(var j=1;j<=12;j++){
    var List,ListText
    List=document.createElement("option");
    ListText=document.createTextNode(j);
    List.appendChild(ListText);
    document.getElementById("st_02").appendChild(List);
    if(j==1){
      List.selected="selected";
    }
  }
  ...
  for(var j=1;j<=31;j++){
    ...
    ListText=document.createTextNode(j);
    List.appendChild(ListText);
    document.getElementById("st_03").appendChild(List);
    if(j==1){
      List.selected="selected";
    }
  }
}
window.onload=function(){OpYear();}
